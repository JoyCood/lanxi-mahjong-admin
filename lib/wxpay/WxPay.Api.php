...
class WxPayApi
{
	...
	{
		//如果不需要上报数据
		if(Config::get('payment', 'wx.report.level') == 0){
			return;
		} 
		//如果仅失败上报
		if(Config::get('payment', 'wx.report.level') == 1 &&
	...
	{		
		...
		//如果有配置代理这里就设置代理
		if(Config::get('payment', 'wx.curl.proxy.host') != "0.0.0.0" 
			&& Config::get('payment', 'wx.curl.proxy.port') != 0){
			curl_setopt($ch,CURLOPT_PROXY, Config::get('payment', 'wx.curl.proxy.host'));
			curl_setopt($ch,CURLOPT_PROXYPORT, Config::get('payment', 'wx.curl.proxy.port'));
		}
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, Config::get('payment', 'wx.sslcert.path'));
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, Config::get('payment', 'wx.sslkey.path'));
		}
		//post提交方式
